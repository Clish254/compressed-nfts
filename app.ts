...
async function uploadJsonToPinata(jsonData: any) {
  try {
    ...
    const pinataBody = {
      ...
      pinataMetadata: {
        name: "Fellows NFT Metadata",
      },
    };

    const response = await axios.post(url, pinataBody, {
      headers: {
...
async function main() {
  ...
  const collectionMetadataUri = await uploadJsonToPinata(collectionMetadata);
  ...
  // this is to avoid AccountNotInitialized errors which i believe are caused
  // by sending the mint instruction before the collection creation transaction
  // is finalized
  ...
  for (const [index, fellow] of fellows.entries()) {
    ...
    const nftItemJsonUri = await uploadJsonToPinata(nftItemJsonObject);
